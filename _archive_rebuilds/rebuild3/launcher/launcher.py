...
LAUNCHER_HOST = os.environ.get('REBUILD3_LAUNCHER_HOST', '127.0.0.1')
LAUNCHER_PORT = int(os.environ.get('REBUILD3_LAUNCHER_PORT', '47120'))
BACKEND_HOST = os.environ.get('REBUILD3_BACKEND_HOST', '127.0.0.1')
BACKEND_PORT = int(os.environ.get('REBUILD3_BACKEND_PORT', '47121'))
FRONTEND_HOST = os.environ.get('REBUILD3_FRONTEND_HOST', '127.0.0.1')
FRONTEND_PORT = int(os.environ.get('REBUILD3_FRONTEND_PORT', '47122'))
DB_HOST = os.environ.get('REBUILD3_PG_HOST', '192.168.200.217')
DB_PORT = int(os.environ.get('REBUILD3_PG_PORT', '5433'))
...
def backend_health() -> dict[str, Any] | None:
    try:
        with urlopen(f'http://{BACKEND_HOST}:{BACKEND_PORT}/api/v1/health', timeout=0.5) as response:
...
def service_snapshot(service_key: str) -> dict[str, Any]:
    ...
    snapshot = {
        ...
        'links': {
            ...
            'health_url': f'http://{service.host}:{service.port}/api/v1/health' if service.key == 'backend' else None,
        },
    }
...
def handle_service_action(service_key: str, action: str) -> dict[str, Any]:
    ...
    if action == 'kill-port':
        ...
        script = {
            'start': service.start_script,
            'stop': service.stop_script,
            'restart': service.restart_script,
        }.get(action)
        if script is None:
            return {
                'status': 'error',
...
def handle_all_action(action: str) -> dict[str, Any]:
    ...
    if action == 'start':
        ...
        results = [handle_service_action(name, 'start') for name in order]
    elif action == 'stop':
        order = ['frontend', 'backend']
        results = [handle_service_action(name, 'stop') for name in order]
    else:
        stop_results = [handle_service_action(name, 'stop') for name in ['frontend', 'backend']]
        start_results = [handle_service_action(name, 'start') for name in ['backend', 'frontend']]
        results = stop_results + start_results

    return {
        'status': 'ok' if all(item['status'] == 'ok' for item in results) else 'error',
        'action': action,
        'message': f'批量操作完成：{action}',
        'results': results,
        'services': [service_snapshot(name) for name in SERVICE_REGISTRY],
    }


def launcher_payload(host: str, port: int) -> dict[str, Any]:
    # 并行获取服务快照，避免串行端口检测导致慢
    ...
    return {
        ...
        'quick_links': {
            ...
            'backend_health': f'http://{BACKEND_HOST}:{BACKEND_PORT}/api/v1/health',
            'runtime_guide': '/docs/runtime',
        },
...
class LauncherHandler(BaseHTTPRequestHandler):
    server_version = 'rebuild3-launcher/0.1'

    def log_message(self, fmt: str, *args: Any) -> None:
        return

    @property
    def typed_server(self) -> ThreadingHTTPServer:
    ...
    def _send_text(self, content: str, content_type: str = 'text/plain; charset=utf-8', status: int = HTTPStatus.OK) -> None:
    ...
    def do_GET(self) -> None:
        ...
        if path in {'/', '/index.html'}:
            ...
            return
        if path == '/docs/runtime':
            guide = PROJECT_ROOT / 'docs' / 'runtime_startup_guide.md'
            self._send_text(guide.read_text() if guide.exists() else 'runtime guide missing')
            return
...
def main() -> None:
    parser = argparse.ArgumentParser(description='rebuild3 standalone launcher')
    ...
    if not args.no_browser:
        threading.Timer(0.6, launch_browser, args=(url,)).start()
