# `Y_codex_Layer5_BS_Profile` 字段说明（BS 画像表）

> 说明：底表字段名为中文；如需英文列名可用视图 `public."Y_codex_Layer5_BS_Profile_EN"`。
...
| BS | bs_id_final | BS 十进制（Layer_4 推导 `bs_id_final`） |
| 物理分桶BS键 | wuli_fentong_bs_key | 物理分桶键（用于多运营商共站/分桶分析） |
...
| 疑似碰撞标记 | is_collision_suspect | 该 BS 是否命中“疑似碰撞”（来自 Layer_4 明细字段 `is_collision_suspect`，聚合取 max>0） |
| 严重碰撞桶标记 | is_severe_collision | 该 BS 是否命中“严重碰撞桶”（来自 Layer_4 明细字段 `is_severe_collision`，聚合取 max=true） |
...
| 移动CELL去重数 | dynamic_cell_cnt | 该 BS 下被判定为“移动/动态 Cell”的去重 Cell 数（来自 Layer_3 Step35 结果映射） |
| 含移动CELL标记 | has_dynamic_cell | 是否存在移动/动态 Cell：`移动CELL去重数>0` |
