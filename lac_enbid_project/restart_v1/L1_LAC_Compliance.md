# L1-LAC 合规规则与当前结论（restart_v1）

> 目的：在第 0 层原始数据解析的基础上，为第 1 层构建一套 **“LAC 合规规则”**，从原始数据中抽取“尽量合规”的 LAC 子集，为后续 CELL/GPS 合规和 L2 分析提供稳定基础。

本文件描述的都是 **第 1 层（L1）的 LAC 部分**。CELL/GPS 合规将分别在后续的 `L1_CELL_Compliance.md`、`L1_GPS_Compliance.md` 中补充。
...
在 L1-LAC 阶段，我们只关注以下 5 个国内 PLMN：
...
在原始表 `public."网优cell项目_清洗补齐库_v1"` 的基础上，我们定义了一个只针对 LAC 的视图，用于派生必要字段并标记异常：
...
通过 `tech_norm` 和 `tech_bucket`，我们把制式归成三个大类：
...
- `OTHER` 类（未来如出现）默认视为“不合规”或“需单独分析”。
...
- `0` / `-1` / 非数字在核心 PLMN 内几乎没有，若在后续扩展 PLMN 时出现，则直接按“不合规”处理。

在此基础上，可定义一个“L1-LAC 合规子集”视图：
...
- 对 **联通 46001 / 电信 46011**：
  ...
  - **不能**简单用“4=4G、6=5G”这样的规则推断制式，`hex_len` 只能作为辅助特征，真正的制式判断仍依赖 `tech` 字段。

**L1 决策：**

- 在 L1 阶段，长度不作为硬性“合/不合规”标准，而是：  
  - 对移动系可用 `hex_len` 做强约束（优先选 4/6 位，标记 3/5 位为可疑）；  
  - 对联通/电信仅做“长度异常”检查（例如出现 1 位、2 位、>6 位时视为明显不合规），当前数据中未见极端情况。

---

## 5. 4G/5G 跨运营商 LAC 重合的解释

为理解 LAC 共享/碰撞的背景，我们把运营商分成两组：
...
- 4G 时代的 LAC（通常对应 4 位 16 进制）可用的编码空间相对较小；  
  不同运营商在全国范围内都会用到相同的十进制 LAC 值，**导致跨运营商之间自然存在 20%左右的“值上的重合”**；  
  这并不意味着物理层面共网或共享小区。
- 5G 时代 TAC 扩展到 6 位 16 进制，可用空间大很多，  
  各运营商可以采取各自独立的编码体系，跨系统碰撞比例自然降到个位数。
- 因此：**LAC 值本身不能作为唯一键来区分运营商或物理区域**，后续必须结合 `network_group`、`cell_id` 和 GPS 来确定真实小区。

在 L1-LAC 阶段，这个结论主要用于提醒：  
**不要试图用“LAC 是否共享”本身去判定数据质量，而应更多依赖制式、位置和时间维度。**

---

## 6. L1-LAC 合规层的输出与后续衔接

### 6.1 当前 L1-LAC 输出（供后续使用）
...
后续 L1-CELL/L1-GPS 可直接在 `v_lac_L1_compliant` 或 `v_lac_L1_compliant_strict` 上继续做合规筛选。
...
- **L1-GPS 合规**：  
  ...
  - 得到 L1-三重合规数据，为后续 L2 分析提供高质量样本。

到这里，L1-LAC 部分的合规规则与关键结论已经记录完毕，可以作为重启后的固定起点。后续重启分析时，只需先在数据库里重建这些视图，再从 `v_lac_L1_compliant` / `v_lac_L1_compliant_strict` 开始继续向下做 CELL/GPS 和 L2 分析即可。
