...
-- Step30 分片并发执行（psql 专用）
-- 目的：绕过 PG 查询内并行的 leader 单线程瓶颈（MessageQueueSend），用“多会话并发 + 分片过滤”吃满多核。
--
-- 用法（示例 16 分片）：
--   psql "$DATABASE_URL" -v shard_count=16 -v shard_id=0 -f lac_enbid_project/Layer_3/sql/30_step30_master_bs_library_shard_psql.sql &
--   ...
--   psql "$DATABASE_URL" -v shard_count=16 -v shard_id=15 -f lac_enbid_project/Layer_3/sql/30_step30_master_bs_library_shard_psql.sql &
--
-- 说明：
-- - 本脚本只生成“分片主表”，不生成 Step30_Gps_Level_Stats（避免重复统计）；合并与统计用 `31_step30_merge_shards_psql.sql`。
-- - 依赖：`lac_enbid_project/Layer_3/sql/30_step30_master_bs_library.sql` 已内置 shard filter（codex.shard_count/shard_id）。
...
-- 输出表名：
-- - 推荐：由外层 shell 传入 `-v step30_master_table="\"Y...__shard_00\""`，避免依赖 \gset
-- - 兼容：若未传入，则根据 shard_id 自动生成（依赖 \gset）
\if :{?step30_master_table}
\else
  SELECT format('"%s"', format('Y_codex_Layer3_Step30_Master_BS_Library__shard_%s', lpad(:'shard_id', 2, '0'))) AS step30_master_table \gset
\endif
\echo Using step30_master_table=:step30_master_table

-- 可选：用 application_name 方便现场采样过滤
\set app_name 'codex_step30|mode=shard|shard=' :shard_id '/' :shard_count
SET application_name = :'app_name';

-- 建议：分片并发时，降低查询内并行，避免 MQ 争用（可按需调整）
SET statement_timeout = 0;
SET jit = off;
SET max_parallel_workers_per_gather = 0;

-- 分片参数注入：Step30 SQL 会读取 current_setting('codex.*')
SELECT set_config('codex.shard_count', :'shard_count', false);
SELECT set_config('codex.shard_id', :'shard_id', false);

-- 你也可以在这里注入 smoke（默认不设置，等同于全量）
...
-- 主体查询：从 Step30 原脚本复制“WITH ... SELECT ...”部分，但输出表名改为分片表名。
...
center_init_all_lon_hist AS (
  SELECT tech_norm, bs_id, lac_dec_final, wuli_fentong_bs_key, lon_bin, count(*)::bigint AS cnt
  FROM seed_points_binned
  GROUP BY 1,2,3,4,5
),
center_init_all_lon_rank AS (
  SELECT h.*,
         sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key ORDER BY h.lon_bin) AS cum_cnt,
         sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key) AS total_cnt
  FROM center_init_all_lon_hist h
),
center_init_all_lat_hist AS (
  SELECT tech_norm, bs_id, lac_dec_final, wuli_fentong_bs_key, lat_bin, count(*)::bigint AS cnt
  FROM seed_points_binned
  GROUP BY 1,2,3,4,5
),
center_init_all_lat_rank AS (
  SELECT h.*,
         sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key ORDER BY h.lat_bin) AS cum_cnt,
         sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key) AS total_cnt
  FROM center_init_all_lat_hist h
),
center_init_all_lon AS (
  SELECT
    lo.tech_norm, lo.bs_id, lo.lac_dec_final, lo.wuli_fentong_bs_key,
...
center_init_all_lat AS (
  SELECT
    la.tech_norm, la.bs_id, la.lac_dec_final, la.wuli_fentong_bs_key,
...
center_init_all AS (
  SELECT lo.tech_norm, lo.bs_id, lo.lac_dec_final, lo.wuli_fentong_bs_key, lo.all_point_cnt, lo.center_lon_all, la.center_lat_all
  FROM center_init_all_lon lo
  JOIN center_init_all_lat la
    ON la.tech_norm=lo.tech_norm AND la.bs_id=lo.bs_id AND la.lac_dec_final=lo.lac_dec_final AND la.wuli_fentong_bs_key=lo.wuli_fentong_bs_key
),
...
center_init_sig AS (
  SELECT lo.tech_norm, lo.bs_id, lo.lac_dec_final, lo.wuli_fentong_bs_key, lo.sig_point_cnt, lo.center_lon_sig, la.center_lat_sig
...
center_init AS (
  SELECT
    a.tech_norm, a.bs_id, a.lac_dec_final, a.wuli_fentong_bs_key,
    CASE WHEN coalesce(s.sig_point_cnt, 0) >= p.signal_min_points_for_signal_center THEN s.center_lon_sig ELSE a.center_lon_all END AS center_lon_init,
    CASE WHEN coalesce(s.sig_point_cnt, 0) >= p.signal_min_points_for_signal_center THEN s.center_lat_sig ELSE a.center_lat_all END AS center_lat_init
  FROM center_init_all a
  LEFT JOIN center_init_sig s
    ON s.tech_norm=a.tech_norm AND s.bs_id=a.bs_id AND s.lac_dec_final=a.lac_dec_final AND s.wuli_fentong_bs_key=a.wuli_fentong_bs_key
  CROSS JOIN params p
),
point_dist_init AS (
  SELECT
    g.tech_norm, g.bs_id, g.lac_dec_final, g.wuli_fentong_bs_key,
    g.operator_id_raw, g.cell_id_dec, g.lon, g.lat, g.sig_rsrp_clean, g.is_in_signal_seed,
    c.center_lon_init, c.center_lat_init,
    CASE
  ...
  FROM seed_points g
  JOIN center_init c
    ON c.tech_norm=g.tech_norm AND c.bs_id=g.bs_id AND c.lac_dec_final=g.lac_dec_final AND c.wuli_fentong_bs_key=g.wuli_fentong_bs_key
),
metric_init AS (
  SELECT tech_norm, bs_id, lac_dec_final, wuli_fentong_bs_key, max(dist_m_init) AS gps_max_dist_m_init
...
kept_stats AS (
  SELECT
    tech_norm, bs_id, lac_dec_final, wuli_fentong_bs_key,
...
point_after AS (
  SELECT
    p.*,
    CASE WHEN ks.kept_point_cnt > 0 THEN p.is_kept ELSE true END AS is_kept_effective
  FROM point_keep_flag p
  JOIN kept_stats ks
    ON ks.tech_norm=p.tech_norm AND ks.bs_id=p.bs_id AND ks.lac_dec_final=p.lac_dec_final AND ks.wuli_fentong_bs_key=p.wuli_fentong_bs_key
),
center_final AS (
  WITH
  all_center_lon_hist AS (
    SELECT a.tech_norm, a.bs_id, a.lac_dec_final, a.wuli_fentong_bs_key, round(a.lon * p.center_bin_scale)::int AS lon_bin, count(*)::bigint AS cnt
    FROM point_after a CROSS JOIN params p
  ...
  all_center_lat_hist AS (
    SELECT a.tech_norm, a.bs_id, a.lac_dec_final, a.wuli_fentong_bs_key, round(a.lat * p.center_bin_scale)::int AS lat_bin, count(*)::bigint AS cnt
    FROM point_after a CROSS JOIN params p
    WHERE a.is_kept_effective
    GROUP BY 1,2,3,4,5
  ),
  ...
  all_center AS (
    SELECT lo.tech_norm, lo.bs_id, lo.lac_dec_final, lo.wuli_fentong_bs_key, lo.all_kept_cnt, lo.bs_center_lon_all, la.bs_center_lat_all
  ...
  sig_center AS (
    SELECT lo.tech_norm, lo.bs_id, lo.lac_dec_final, lo.wuli_fentong_bs_key, lo.sig_kept_cnt, lo.bs_center_lon_sig, la.bs_center_lat_sig
    FROM sig_center_lon lo
    JOIN sig_center_lat la
      ON la.tech_norm=lo.tech_norm AND la.bs_id=lo.bs_id AND la.lac_dec_final=lo.lac_dec_final AND la.wuli_fentong_bs_key=lo.wuli_fentong_bs_key
  )
  SELECT
    ac.tech_norm, ac.bs_id, ac.lac_dec_final, ac.wuli_fentong_bs_key,
    max(ks.removed_point_cnt)::bigint AS outlier_removed_cnt,
    CASE WHEN coalesce(sc.sig_kept_cnt, 0) >= p.signal_min_points_for_signal_center THEN sc.bs_center_lon_sig ELSE ac.bs_center_lon_all END AS bs_center_lon,
    CASE WHEN coalesce(sc.sig_kept_cnt, 0) >= p.signal_min_points_for_signal_center THEN sc.bs_center_lat_sig ELSE ac.bs_center_lat_all END AS bs_center_lat
  FROM all_center ac
  LEFT JOIN sig_center sc
    ON sc.tech_norm=ac.tech_norm AND sc.bs_id=ac.bs_id AND sc.lac_dec_final=ac.lac_dec_final AND sc.wuli_fentong_bs_key=ac.wuli_fentong_bs_key
  JOIN kept_stats ks
    ON ks.tech_norm=ac.tech_norm AND ks.bs_id=ac.bs_id AND ks.lac_dec_final=ac.lac_dec_final AND ks.wuli_fentong_bs_key=ac.wuli_fentong_bs_key
  CROSS JOIN params p
  GROUP BY
    ac.tech_norm, ac.bs_id, ac.lac_dec_final, ac.wuli_fentong_bs_key,
    ac.bs_center_lon_all, ac.bs_center_lat_all,
    sc.sig_kept_cnt, sc.bs_center_lon_sig, sc.bs_center_lat_sig,
    p.signal_min_points_for_signal_center
),
point_dist_final AS (
  SELECT
    a.tech_norm, a.bs_id, a.lac_dec_final, a.wuli_fentong_bs_key,
    a.cell_id_dec, a.lon, a.lat,
    c.bs_center_lon, c.bs_center_lat, c.outlier_removed_cnt,
    CASE
  ...
  FROM point_after a
  JOIN center_final c
    ON c.tech_norm=a.tech_norm AND c.bs_id=a.bs_id AND c.lac_dec_final=a.lac_dec_final AND c.wuli_fentong_bs_key=a.wuli_fentong_bs_key
  WHERE a.is_kept_effective
),
dist_hist AS (
  SELECT
    d.tech_norm, d.bs_id, d.lac_dec_final, d.wuli_fentong_bs_key,
...
dist_rank AS (
  SELECT
    h.*,
    sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key ORDER BY h.dist_bin_m) AS cum_cnt,
    sum(h.cnt) OVER (PARTITION BY h.tech_norm, h.bs_id, h.lac_dec_final, h.wuli_fentong_bs_key) AS total_cnt
  FROM dist_hist h
),
dist_pcts AS (
  SELECT
    r.tech_norm, r.bs_id, r.lac_dec_final, r.wuli_fentong_bs_key,
...
metric_final AS (
  SELECT
    b.tech_norm, b.bs_id, b.lac_dec_final, b.wuli_fentong_bs_key,
    b.outlier_removed_cnt, b.bs_center_lon, b.bs_center_lat,
    p.gps_p50_dist_m, p.gps_p90_dist_m,
    b.gps_max_dist_m
  FROM metric_final_base b
  LEFT JOIN dist_pcts p
    ON p.tech_norm=b.tech_norm AND p.bs_id=b.bs_id AND p.lac_dec_final=b.lac_dec_final AND p.wuli_fentong_bs_key=b.wuli_fentong_bs_key
),
anomaly_cell_cnt AS (
  SELECT
    s.tech_norm,
...
LEFT JOIN metric_final m
  ON m.tech_norm=b.tech_norm AND m.bs_id=b.bs_id AND m.lac_dec_final=b.lac_dec_final AND m.wuli_fentong_bs_key=b.wuli_fentong_bs_key
LEFT JOIN kept_stats ks
  ON ks.tech_norm=b.tech_norm AND ks.bs_id=b.bs_id AND ks.lac_dec_final=b.lac_dec_final AND ks.wuli_fentong_bs_key=b.wuli_fentong_bs_key
LEFT JOIN anomaly_cell_cnt a
  ON a.tech_norm=b.tech_norm AND a.bs_id=b.bs_id AND a.lac_dec_final=b.lac_dec_final AND a.wuli_fentong_bs_key=b.wuli_fentong_bs_key
CROSS JOIN params p;
