...
独立运维入口，不依赖主系统前后端。
通过 REST API 提供服务控制、状态检查和日志查看。
支持 --kill-port 参数在启动时自动清理端口冲突。
"""
...
LAUNCHER_HOST = os.environ.get('REBUILD5_LAUNCHER_HOST', '127.0.0.1')
LAUNCHER_PORT = int(os.environ.get('REBUILD5_LAUNCHER_PORT', '47230'))
BACKEND_HOST = os.environ.get('REBUILD5_BACKEND_HOST', '127.0.0.1')
...
SERVICE_REGISTRY: dict[str, ServiceSpec] = {
    'backend': ServiceSpec(
        ...
        supports_kill=True,
        note='FastAPI + PostgreSQL Step 1 API。',
        service_url=f'http://{BACKEND_HOST}:{BACKEND_PORT}',
        health_url=f'http://{BACKEND_HOST}:{BACKEND_PORT}/api/system/health',
    ),
    'frontend': ServiceSpec(
        ...
        supports_actions=True,
        supports_kill=True,
        note='Vue 3 + Vite 设计稿联调入口。',
        service_url=f'http://{FRONTEND_HOST}:{FRONTEND_PORT}',
    ),
...
def backend_health() -> dict[str, Any] | None:
    try:
        with urlopen(f'http://{BACKEND_HOST}:{BACKEND_PORT}/api/system/health', timeout=0.5) as response:
...
def kill_pids_gracefully(pids: list[int], exclude: set[int] | None = None) -> tuple[list[int], list[int]]:
    """先 SIGTERM，等 3 秒，仍存活则 SIGKILL。返回 (killed_pids, skipped_pids)。"""
    exclude_set = set(exclude or [])
    to_kill = [p for p in pids if p not in exclude_set]
    skipped = [p for p in pids if p in exclude_set]
    for pid in to_kill:
        try:
    ...
    while time.time() < deadline:
        if not any(pid_alive(p) for p in to_kill):
            break
        time.sleep(0.2)
    for pid in to_kill:
        if pid_alive(pid):
...
def kill_local_port(service_key: str) -> tuple[bool, str]:
    ...
    pids = find_port_occupants(service.port)
    if not pids:
        return True, f'端口 {service.port} 当前空闲'
    killed, _ = kill_pids_gracefully(pids)
    # 等端口真正释放
    ...
    return True, f'已释放端口 {service.port}（处理 {len(killed)} 个进程）'


def kill_launcher_port(host: str, port: int) -> dict[str, Any]:
    """清理 launcher 端口（由 UI 调用，排除自身 PID）。"""
    pids = find_port_occupants(port)
    self_pid = os.getpid()
    if not pids:
        return {'ok': True, 'message': f'端口 {port} 当前没有占用进程。', 'killed_pids': [], 'skipped_pids': []}
    killed, skipped = kill_pids_gracefully(pids, exclude={self_pid})
    if killed:
        msg = f'已发送终止信号到进程: {", ".join(str(p) for p in killed)}'
    elif skipped:
        msg = '仅检测到当前 launcher 进程自身，已跳过。'
    else:
        msg = '没有可清理的外部进程。'
    return {
        'ok': True,
...
def service_snapshot(service_key: str) -> dict[str, Any]:
    ...
    snapshot: dict[str, Any] = {
        ...
        'links': {
            'service_url': service.service_url or (f'http://{service.host}:{service.port}' if is_local_host(service.host) else None),
...
def run_script(script: Path | None) -> tuple[bool, str]:
    ...
    completed = subprocess.run(
        [str(script)], cwd=str(WORKSPACE_ROOT), text=True, capture_output=True, timeout=120,
    )
...
def launcher_payload() -> dict[str, Any]:
    ...
    return {
        ...
        'launcher': {
            'host': LAUNCHER_STATE['host'],
            'port': LAUNCHER_STATE['port'],
            'url': LAUNCHER_STATE['url'],
            'port_conflict': LAUNCHER_STATE['port_conflict'],
        },
        'quick_links': {
            'workbench': f'http://{FRONTEND_HOST}:{FRONTEND_PORT}',
            'backend_health': f'http://{BACKEND_HOST}:{BACKEND_PORT}/api/system/health',
        },
...
class LauncherHandler(BaseHTTPRequestHandler):
    server_version = 'rebuild5-launcher/1.0'
    ...
    def do_GET(self) -> None:
        ...
        if path == '/api/health':
            self._send_json({'status': 'ok', 'service': 'rebuild5-launcher'})
            return

        if path == '/api/launcher/services':
            self._send_json(launcher_payload())
            return
    ...
    def do_POST(self) -> None:
        ...
        # 一键清理 Launcher 端口（原始冲突端口）
        if path == '/api/launcher/cleanup-port':
            conflict_port = int(LAUNCHER_STATE['port'])
            result = kill_launcher_port(LAUNCHER_STATE['host'], conflict_port)
            LAUNCHER_STATE['port_conflict'] = result if result['killed_pids'] else LAUNCHER_STATE['port_conflict']
...
def main() -> None:
    parser = argparse.ArgumentParser(description='rebuild5 独立启动器')
