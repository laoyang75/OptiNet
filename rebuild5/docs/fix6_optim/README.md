...
| **02 审计 + 结构 + 测试** | agent | `02_audit_prompt.md` ← 01 完成后由 Claude 写 | `02_audit_report.md` + 抽出的 `core/citus_compat.py` + 测试套件 | 01 |
| **03 pipelined 加速** | agent | `03_pipelined_prompt.md` ← 02 完成后由 Claude 写 | `03_pipelined_report.md` + `run_citus_pipelined_batches.py` | 02 |
...
- **Claude(我)**:写 prompt + review agent 产出 + 决定下一阶段范围
- **独立 agent**:执行 prompt + 写报告 + 写 notes
- **用户(你)**:拍板边界 + ack agent 开跑前的报告 + 最终 review

每个 prompt 都套 `_prompt_template.md`,不漏字段。
...
- [ ] 01 收尾 — agent 用 `01_finalize_and_commit_prompt.md`
- [ ] 02 审计 + 结构 + 测试 — 等 01
- [ ] 03 pipelined — 等 02
- [ ] 04 Runbook — 等 03
- [ ] UI 支线 — 任何时候
