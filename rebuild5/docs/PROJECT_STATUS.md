...
## 2. 当前 active 的事(2026-04-27 起)

| 事项 | 状态 |
|---|---|
| 内核升级 216(单机试验) | prompt / report 已归档:`archive/fix_history/upgrade/kernel_upgrade_216_prompt.md` |
| UI 修复(8 块累积清洗规则前端落地) | 部分 Claude 直接做 + 部分独立 agent prompt 待写 |
| 文档收尾(本批) | PROJECT_STATUS.md(本) + runbook.md(顶层) + 各 trail 子目录历史标记 |
| 数仓搭建 | 推迟,稳定期后单独立项 |
...
- 内核升级**仅 216 试验**走 § 3.1.1 不 drain,**保旧内核** 3.10 作 GRUB 回退入口
- **绝不删旧内核 / 旧数据**
