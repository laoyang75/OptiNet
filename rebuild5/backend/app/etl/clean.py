...
ODS_RULES = [
    {"id": "ODS-001", "name": "垃圾运营商编码置空", "field": "operator_code", "action": "nullify", "where": "operator_code IN ('00000','0','000000','(null)(null)','')", "desc": "明确的垃圾运营商编码"},
    ...
    {"id": "ODS-006", "name": "CellID=0 置空", "field": "cell_id", "action": "nullify", "where": "cell_id = 0", "desc": "无有效小区标识"},
    {"id": "ODS-007", "name": "5G CellID 溢出值置空", "field": "cell_id", "action": "nullify", "where": "cell_id = 268435455 AND tech_norm = '5G'", "desc": "5G CellID 溢出值"},
    {"id": "ODS-008", "name": "4G CellID 溢出值置空", "field": "cell_id", "action": "nullify", "where": "cell_id = 2147483647 AND tech_norm = '4G'", "desc": "4G CellID 溢出值"},
    ...
    {"id": "ODS-013", "name": "非原生 GPS 标记无效", "field": "gps_valid", "action": "flag_gps", "where": "cell_origin = 'cell_infos' AND (gps_info_type IS NULL OR gps_info_type NOT IN ('gps', '1'))", "desc": "仅信任原生 GPS"},
    {"id": "ODS-014", "name": "非原生 GPS 经纬度置空", "field": "lon_raw", "action": "nullify_pair", "where": "cell_origin = 'cell_infos' AND gps_valid = false AND lon_raw IS NOT NULL", "desc": "无效 GPS 坐标不参与空间计算"},
...
def step1_clean() -> dict[str, Any]:
    ...
    for rule in ODS_RULES:
        ...
        rule_stats.append({
            ...
            'deleted_rows': violations if rule['id'] in {'ODS-006', 'ODS-007', 'ODS-008'} else 0,
    ...
    filtered_count = int(deleted['cnt']) if deleted else 0
