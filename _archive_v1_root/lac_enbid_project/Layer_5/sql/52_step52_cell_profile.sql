-- Layer_5 Step52：CELL Profile（按 CELL 汇总一行一对象）
--
-- 输入：
...
DROP TABLE IF EXISTS public."Y_codex_Layer5_Cell_Profile";

CREATE TABLE public."Y_codex_Layer5_Cell_Profile" AS
WITH
params AS (
  SELECT
    200::bigint AS min_rows_for_profile,
...
SELECT
  ...
  a.is_dynamic_cell AS "移动CELL标记",
  a.dynamic_reason AS "移动原因",
  a.half_major_dist_km AS "移动半长轴KM",
...
ANALYZE public."Y_codex_Layer5_Cell_Profile";
