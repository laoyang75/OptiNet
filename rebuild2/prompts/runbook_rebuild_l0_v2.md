...
> 身份：**执行 Agent**
> 任务：修正 ss1 信号匹配逻辑 + 去除邻区行 + 去除 is_connected 列 + WiFi 清洗，重建 l0_gps 和 l0_lac
...
- **SSH**：`ssh root@192.168.200.217`，密码 `111111`
...
| # | 变更 | 原因 |
|---|------|------|
| 1 | ss1 信号按制式匹配基站 | 之前把第一条信号统一给所有基站，错误 |
| 2 | 去掉 is_connected=2 的行 | 邻区无 CellID，无业务意义 |
| 3 | 去掉 is_connected 列 | 只剩主服务(1)和ss1(null)，列无意义 |
| 4 | WiFi 无效值清洗 | `<unknown ssid>` 和 `02:00:00:00:00:00` 置 NULL |
| 5 | ss1 信号 RSRP/RSRQ/RSSNR/SS = -1 置 NULL | -1 表示未获取 |

---

## 执行流程：MCP 测试 → SSH 全量 → MCP 验证

---

### STEP 0：MCP 测试 ss1 新解析逻辑

**目的**：验证信号按制式匹配基站的 SQL 正确性。

用 MCP 执行以下 SQL（小规模测试）：

```sql
-- 取 10 条有双卡基站的 ss1 记录测试
WITH test_data AS (
  SELECT "记录数唯一标识" AS record_id, "ss1"
  FROM legacy."网优项目_gps定位北京明细数据_20251201_20251207"
  WHERE "ss1" IS NOT NULL AND "ss1" LIKE '%+l,%' AND "ss1" LIKE '%+n,%'
  LIMIT 10
),
groups AS (
  SELECT record_id, grp, grp_idx
  FROM test_data,
  LATERAL unnest(string_to_array(trim(trailing ';' FROM "ss1"), ';'))
...
with_carry AS (
  SELECT e.*,
    ...
    CASE WHEN cell_block = '1' THEN 'inherited' WHEN cell_block NOT IN ('', '0', '1') THEN 'own' ELSE 'none' END AS cb_source
  FROM elements e
),
inherited AS (
  SELECT w.*,
...
-- 拆信号
sigs AS (
  SELECT h.*, unnest(string_to_array(rtrim(sig_block, '+'), '+')) AS sig_entry
  FROM inherited h
  WHERE sig_block IS NOT NULL AND sig_block != ''
),
-- 解析基站
parsed_cells AS (
  SELECT record_id, grp_idx, ts_block, gps_block, cb_source, sig_block,
...
-- 按制式匹配：cell LEFT JOIN signal ON 同组同制式
matched AS (
  SELECT
    c.record_id, c.grp_idx, c.cell_tech, c.cid_str, c.lac_str, c.plmn_str,
    c.ts_block, c.gps_block, c.cb_source,
...
SELECT * FROM matched ORDER BY record_id, grp_idx LIMIT 30;
```

**✅ 检查点 0**：确认输出中：
- 同一 grp_idx 下，NR 基站绑了 NR 信号（或 NULL），LTE 基站绑了 LTE 信号
- 双卡场景下两个基站各自有独立的信号行
- CellID=-1 的基站已被过滤
...
**✅ 检查点 1**（用 MCP 查）：
```sql
SELECT count(*) FROM rebuild2.l0_gps_ci;
-- 预期：约 2000~2500 万行（之前主服务约 17% × 1.27 亿 ≈ 2160 万）
```
...
**✅ 检查点 2**（MCP 查）：
```sql
SELECT count(*) FROM rebuild2.l0_gps_ss1;
-- 预期：约 1200~1400 万行
```
...
SELECT count(*) AS total,
  ...
  count(*) FILTER (WHERE "CellID" IS NULL) AS no_cellid
FROM rebuild2.l0_gps_new;
-- 预期：total 约 3000~3500 万，has_cellid = total（100%），no_cellid = 0
```
...
```sql
-- 确认索引已创建
SELECT indexname FROM pg_indexes WHERE schemaname = 'rebuild2' AND tablename = 'l0_gps' ORDER BY indexname;
-- 预期：6 个索引
```

---

### STEP 6：同样逻辑重建 l0_lac

**重复 STEP 2~4，做以下替换：**

| 原文 | 替换为 |
|------|--------|
| `网优项目_gps定位北京明细数据_20251201_20251207` | `网优项目_lac定位北京明细数据_20251201_20251207` |
| `rebuild2.l0_gps_ci` | `rebuild2.l0_lac_ci` |
| `rebuild2.l0_gps_ss1` | `rebuild2.l0_lac_ss1` |
| `rebuild2.l0_gps_new` | `rebuild2.l0_lac_new` |

然后替换旧表 + 建索引：

```sql
DROP TABLE IF EXISTS rebuild2.l0_lac;
ALTER TABLE rebuild2.l0_lac_new RENAME TO l0_lac;

CREATE INDEX idx_l0_lac_cellid ON rebuild2.l0_lac ("CellID") WHERE "CellID" IS NOT NULL;
CREATE INDEX idx_l0_lac_lac ON rebuild2.l0_lac ("LAC") WHERE "LAC" IS NOT NULL;
CREATE INDEX idx_l0_lac_bsid ON rebuild2.l0_lac ("基站ID") WHERE "基站ID" IS NOT NULL;
CREATE INDEX idx_l0_lac_operator_tech ON rebuild2.l0_lac ("运营商编码", "标准制式") WHERE "运营商编码" IS NOT NULL;
CREATE INDEX idx_l0_lac_record ON rebuild2.l0_lac ("原始记录ID");
CREATE INDEX idx_l0_lac_ts ON rebuild2.l0_lac ("上报时间") WHERE "上报时间" IS NOT NULL;
...
-- 预期：total 约 2500~3000 万，has_cellid = total（100%），no_cellid = 0
```
...
全量执行完后，用 MCP 执行以下 SQL 刷新 l0_stats_cache（跟之前一样的统计逻辑，但数据变了）：

```sql
-- 清空旧缓存
DELETE FROM rebuild2_meta.l0_stats_cache;
...
-- 同样对 l0_lac 执行以上 4 条（替换表名即可）
```

**✅ 检查点 6**（MCP 查）：
```sql
SELECT table_name, stat_type, stat_key,
  (stat_value->>'total')::bigint AS total
FROM rebuild2_meta.l0_stats_cache
WHERE stat_type = 'summary';
-- 预期：两张表都有数据，total > 0，且 CellID 覆盖率 = 100%
```
...
SELECT 'l0_gps' AS tbl, count(*) AS total,
  count(*) FILTER (WHERE "有CellID") AS has_cellid,
  round(count(*) FILTER (WHERE "有CellID")::numeric / count(*), 4) AS cellid_rate,
  count(*) FILTER (WHERE "RSRP" IS NOT NULL) AS has_rsrp,
  count(DISTINCT "原始记录ID") AS records
FROM rebuild2.l0_gps
UNION ALL
SELECT 'l0_lac', count(*),
  count(*) FILTER (WHERE "有CellID"),
  round(count(*) FILTER (WHERE "有CellID")::numeric / count(*), 4),
...
**✅ 最终检查**：
- CellID 覆盖率 = **100%**（不再有无 CellID 的行）
- 没有 is_connected 列
- 总行数约 3000~3500 万（gps） / 2500~3000 万（lac）
